...
def sparsity_learning_train(opt, model, prune,train_loader, val_loader, device, callbacks):
...
def parse_opt(known=False):
    ...
    parser.add_argument("--sl_hyp", type=str, default="data/hyps/hyp.scratch-low.yaml", help="hyperparameters path for sparsity learning")
...
if __name__ == "__main__":
    ...
    if pretrained:
        ...
        model = torch_load(weights, map_location=device)
        if model["ema"]:
        ...
        # for c2f
        # replace_c2f_with_c2f_v2(model.model) # 1
        model.to(device)
    ...
    # prune dataloader
    prune_loader, _ = create_dataloader(
        train_path,
        imgsz,
        opt.batch_size // WORLD_SIZE // 2,
        gs,
    ...
    if sparsity_learning:
        model = torch_load(opt.sl_model, map_location=device)
