...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov5s.yaml", ch=3, nc=None, anchors=None):
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Segment,yolov8Detect)):
            s = 640  # 2x min stride
...
def parse_model(d, ch):
    ...
    for i, (f, n, m, args) in enumerate(
        ...
        if m in {
            Conv,GhostConv,Bottleneck,GhostBottleneck,SPP,SPPF,DWConv,MixConv2d,Focus,CrossConv,BottleneckCSP,C3,C3TR,C3SPP,C3Ghost, nn.ConvTranspose2d,DWConvTranspose2d,C3x,SimSPPF,ASPP,RFB,SPPCSPC,SPPCSPC_group,SimCSPSPPF,C2f,SPPF_LSKA,HWD,RepBlock,CSPStage,RCSOSA,C3_Faster,C2f_Faster,C2f_ODConv,C3_ODConv,C2f_DBB,C3_DBB,VoVGSCSP,C2f_SCConv,C3_SCConv,C3_ScConv,C2f_ScConv,C3_EMSC,C3_EMSCP,C2f_EMSC,C2f_EMSCP,C2f_KW,C3_KW,C3_DySnakeConv,C2f_DySnakeConv,C2f_DCNv2,C3_DCNv2,C2f_OREPA,C3_OREPA,C3_REPVGGOREPA,C2f_REPVGGOREPA,C3_ContextGuided,C2f_ContextGuided,C2f_MSBlock,C3_MSBlock,C3_DLKA,C2f_DLKA,C3_Parc, C2f_Parc,C3_DWR,C2f_DWR,C3_RFCBAMConv,C3_RFCAConv,C2f_RFCBAMConv,C2f_RFCAConv,C3_AKConv,C2f_AKConv,RepNCSPELAN4,DPBlock,ES_Bottleneck,Dense,conv_bn_relu_maxpool,Shuffle_Block,conv_bn_act,SEAM,MultiSEAM,RFEM,RepConv
        }:
            c1, c2 = ch[f], args[0]
        ...
        # TODO: channel, gw, gd
            ...
            args.append([ch[x] for x in f])
            args.insert(1,[list(range(2))] * len(f))
        elif m is Contract:
            c2 = ch[f] * args[0] ** 2
        elif m is Expand:
            c2 = ch[f] // args[0] ** 2
